# Derivation of the volume integral form of the Eulerian heat budget
...
\frac{d}{dt}\Gamma(t) &= \int_{V(t)}\frac{\partial \gamma}{\partial t} dV + \iint_A \gamma (\vec v_{b}\cdot\hat n) dA.
\end{align*}
$$
...
$$
\frac{d}{dt}\int_{V(t)}TdV = \int_{V(t)}\bigg[ -\vec \nabla \cdot(\vec U T) + \omega\frac{RT}{c_P P} + \frac{J}{c_P}\bigg]dv + \iint_{\text{surface}} T (\vec v_{b}\cdot\hat n) dA
$$
Applying Gauss' Law:
$$
\frac{d}{dt}\int_{V(t)}TdV = -\iint_A T(\vec U \cdot \hat n)dA + \int_{V(t)}\bigg[\omega\frac{RT}{c_P P} + \frac{J}{c_P}\bigg]dv + \iint_{\text{surface}} T (\vec v_{b}\cdot\hat n) dA
$$
If we expand the surface integral of the advection components:
$$
\iint_A T(\vec U \cdot \hat n) dA = \iint_{top+west+east+north+south}T(\vec U \cdot \hat n) dA + \iint_{\text{surface}} T(\vec U \cdot \hat n) dA.
$$
At the surface, there is no transfer of mass across the boundary,
$$
\vec U \cdot \hat n\bigg|_{surface}=v_b \cdot \hat n\bigg|_{surface}.
$$
...
$$
where minus signs in the mass-balance equation indicate that the mass-residual is positive for transport into the box.
...
$$
\frac{d\langle T\rangle}{dt} = -\frac{1}{V}\iint_{T+W+E+N+S} T'(\vec U\cdot \hat n)dA +\frac{1}{V}\int_{V(t)}\bigg[\omega\frac{RT}{c_P P} + \frac{J}{c_P}\bigg]dV
$$
...
$$
\delta \mathcal{H} \equiv \langle T\rangle \bigg[-\iint(\vec U\cdot\hat n)dA - \frac{dV}{dt} \bigg ] = \langle T\rangle \delta \mathcal{M}
$$
