...
- On Venus, keep commit-specific production checkouts under
  `~/eulerian-heat-budget/production/` and all production outputs and logs
  under `~/eulerian-heat-budget/campaign-data/`.
- Never switch, fetch, pull, or modify a production checkout referenced by a
  queued or running job. Create a new checkout for a new commit.
