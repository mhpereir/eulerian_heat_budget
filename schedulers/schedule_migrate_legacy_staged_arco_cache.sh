...
mkdir -p "${LOG_DIR}"
LOGFILE="${LOG_DIR}/${JOB_ID}_EHB_migrate_legacy.log"
...
: "${PBS_ARRAY_INDEX:?PBS_ARRAY_INDEX must be set for yearly migration}"
...
echo "[info] $(date -Is) starting legacy cache migration for year ${YEAR} on host $(hostname)"
echo "[info] repo root: ${PROJECT_ROOT}"
echo "[info] expected commit: ${EXPECTED_COMMIT}"
echo "[info] legacy cache root: ${LEGACY_CACHE_ROOT}"
...
/usr/bin/time -v "${PYTHON_EXECUTABLE}" -u migrate_legacy_staged_arco_cache.py \
  --legacy-cache-root "${LEGACY_CACHE_ROOT}" \
  --campaign-root "${STAGED_CACHE_ROOT}" \
  --year "${YEAR}" \
  --git-commit "${EXPECTED_COMMIT}" \
  --pbs-job-id "${PBS_JOBID:-}" \
  --pbs-array-index "${PBS_ARRAY_INDEX}"
echo "[info] $(date -Is) finished legacy cache migration for year ${YEAR}"
