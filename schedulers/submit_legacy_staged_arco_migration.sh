#!/bin/bash
# Submit a legacy-cache migration array and optional dependent consolidation.

set -euo pipefail

PROJECT_ROOT="${PROJECT_ROOT:?PROJECT_ROOT must identify the clean Venus checkout}"
LEGACY_CACHE_ROOT="${LEGACY_CACHE_ROOT:?LEGACY_CACHE_ROOT must identify the read-only source cache}"
...
if [[ "$(readlink -m -- "${LEGACY_CACHE_ROOT}")" == "$(readlink -m -- "${STAGED_CACHE_ROOT}")" ]]; then
  echo "[error] legacy source and campaign destination must differ." >&2
  exit 2
fi
if [[ ! -x "${QSUB_BIN}" ]]; then
...
EXPORTS+=",LEGACY_CACHE_ROOT=${LEGACY_CACHE_ROOT}"
...
EXPORTS+=",VENUS_MAMBA_ENV=${VENUS_MAMBA_ENV:-dev_env},LOG_DIR=${LOG_DIR}"
...
MIGRATION_JOB_ID=$(
  "${QSUB_BIN}" \
    ...
    -o "${LOG_DIR}/" \
    -v "${EXPORTS}" \
    "${SCHEDULER_DIR}/schedule_migrate_legacy_staged_arco_cache.sh"
)
echo "[submit] migration_job_id=${MIGRATION_JOB_ID}"

if ehb_bool_enabled "${SUBMIT_CONSOLIDATION}"; then
  CONSOLIDATION_JOB_ID=$(
    "${QSUB_BIN}" \
      -W "depend=afterok:${MIGRATION_JOB_ID}" \
      -o "${LOG_DIR}/" \
      -v "${EXPORTS}" \
      "${SCHEDULER_DIR}/schedule_consolidate_legacy_staged_arco_cache.sh"
  )
